...
namespace JassToTs
{
    class JassToLua
    {
        ...
        public JassToLua(bool isOptimizationNeeded = false, int IndentSize = 4)
        {
        ...
        StringBuilder ConvertDeclarations(Statement stat)
        {
            switch (stat.Type)
            {
                case StatementType.Comm:
                    return ConvertComm(stat).Append("\n");
        ...
        StringBuilder ConvertTypeDecl(Statement tree)
        {
            ...
            for (var i = 0; i < tree.Childs.Count; i++)
            {
                ...
                {
                    case StatementType.Comm:
                        comm = ConvertComm(comm, tree.Childs[i]).Append('\n');
                        continue;
            ...
            return sb.Append("-- ").Append(typeName).Append(" extends ").Append(baseType).Append(comm);
        }
        ...
        StringBuilder ConvertLocals(Statement stat, int indent = 0)
        {
            ...
            {
                case StatementType.Comm:
                    return AddIndent(ConvertComm(stat), indent).Append(stat.Start.Text).Append("\n");
        ...
        StringBuilder ConvertVarDecl(Statement tree, int indent = 0)
        {
            ...
            for (var i = 0; i < tree.Childs.Count; i++)
            {
                ...
                {
                    case StatementType.Comm:
                        comm = ConvertComm(tree.Childs[i]).Append('\n');
                        continue;
        ...
        StringBuilder ConvertExprElem(Statement elem)
        {
            ...
            {
                case StatementType.Comm:
                    return ConvertComm(elem).Append('\n');
                case StatementType.RVar:
                case StatementType.RFunc:
        ...
        StringBuilder ConvertFuncCall(Statement tree)
        {
            ...
            for (var i = 0; i < tree.Childs.Count; i++)
            {
                ...
                {
                    case StatementType.Comm:
                        return ConvertComm(sb, tree.Childs[i]).Append('\n');
                    case StatementType.Name:
                        name = tree.Childs[i].Start.Text;
                        continue;
                    default:
        ...
        StringBuilder ConvertFunc(Statement tree)
        {
            ...
            for (var i = 0; i < tree.Childs.Count; i++)
            {
                ...
                {
                    case StatementType.Comm:
                        sb = ConvertComm(sb, tree.Childs[i]).Append('\n');
                        continue;
        ...
        StringBuilder ConvertComm(Statement tree)
        {
            var sb = new StringBuilder();
            sb.Append("--").Append(tree.Start.Text.Substring(2)).Append('\n');
            return sb;
        }

        StringBuilder ConvertComm(StringBuilder sb, Statement tree)
        {
            sb.Append("--").Append(tree.Start.Text.Substring(2)).Append('\n');
            return sb;
        }
        ...
        StringBuilder ConvertFuncDecl(Statement tree)
        {
            ...
            for (var i = 0; i < tree.Childs.Count; i++)
            {
                ...
                {
                    case StatementType.Comm:
                        ConvertComm(sb, tree.Childs[i]).Append('\n');
                        continue;
                    case StatementType.Name:
                        name = tree.Childs[i].Start.Text;
                        continue;
        ...
        StringBuilder ConvertStatement(Statement stat, int indent = 0)
        {
            ...
            {
                case StatementType.Comm:
                    return AddIndent(sb, indent).Append("--").Append(stat.Start.Text.Substring(2)).Append('\n');
                ...
                case StatementType.Loop:
                    ...
                    AddIndent(sb, indent).Append("do\n").AppendJoin("", stat.Childs.Select(x => ConvertStatement(x, indent + 1)));
                    AddIndent(sb, indent).Append("end\n");
                    return sb;
                case StatementType.Exit:
                    return AddIndent(sb, indent).Append("if (").Append(ConvertExpr(stat)).Append(") then break end\n");
        ...
        StringBuilder ConvertIfStatement(Statement tree, int indent = 0)
        {
            ...
            for (var i = 0; i < tree.Childs.Count; i++)
            {
                ...
                {
                    case StatementType.Comm:
                        sb = AddIndent(sb, indent);
                        sb = ConvertComm(sb, tree.Childs[i]).Append('\n');
                        continue;
                    case StatementType.Cond:
                        sb.Append("if (").Append(ConvertExpr(tree.Childs[i])).Append(") then\n");
                        continue;
                    case StatementType.ElseCond:
